...
void visionAdjust(double *param_Adjust, bool *adjust_Finished)
{
    ...
    {
        /*Move Closer*/
        if((TerrainAnalysis::leftedge_z[0] < 30) || (TerrainAnalysis::leftedge_z[0] > 32))
        {
            cout<<"MOVE FORWARD AND BACKWARD!"<<endl;
            double movez_data[3] = {0, 0, 0};
            movez_data[2] = (TerrainAnalysis::leftedge_z[0] - 31)*0.025;
            /*max walk*/
            if(TerrainAnalysis::leftedge_z[0] < 30)
            {
